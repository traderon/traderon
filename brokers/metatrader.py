...
def metatrader_import(api_key, account_ID):
    "type message 'connecting account from metatrader'"
    login='571182'
    password='tjcJQ80##'
    meta_type='mt5'
    passphrase='GrowthNext-Server'
    headers ={
    ...
    response = requests.request("GET", "https://mt-provisioning-api-v1.agiliumtrade.agiliumtrade.ai/users/current/accounts?offset=0&limit=1000&endpointVersion=v1", headers=headers).json()
    accountId=''
    isCreated=False
    ...
    if response !=[]:
        for trade in response:
            if trade['login']==login:
                isCreated=True
                account=trade['_id']
                response = requests.request("GET", "https://mt-provisioning-api-v1.agiliumtrade.agiliumtrade.ai/users/current/accounts/"+account, headers=headers).json()
                ...
                if response["state"]=="UNDEPLOYED":
                    requests.request("POST", "https://mt-provisioning-api-v1.agiliumtrade.agiliumtrade.ai/users/current/accounts/"+account+"/deploy?executeForAllReplicas=true", headers=headers)
                    cont=0
                    response = requests.request("GET", "https://mt-provisioning-api-v1.agiliumtrade.agiliumtrade.ai/users/current/accounts/"+account, headers=headers).json()
    ...
    if not isCreated:
        ...
        try:
            ...
            if 'error' in response:
                return "Error connecting account, please verify your credentials"
            account=response['id']
            tt.sleep(30)
...
def get_metatrader_orders():
 ...
 #response = requests.request("POST", "https://mt-provisioning-api-v1.agiliumtrade.agiliumtrade.ai/users/current/accounts", headers=headers,json=params).json()
         #r = requests.get(url="https://mt-client-api-uzjylcntlrgwy9nc.new-york.agiliumtrade.ai/users/current/accounts/"+self.account+"/connected",headers=headers).json()
            ...
            while(paginated):
                 r = requests.get(url='https://mt-client-api-v1.'+region+'.agiliumtrade.ai/users/current/accounts/'+account+'/history-deals/time/'+startDate+startHora+'/'+endDate+endHora,headers=headers,params=paramQuery).json()
                 offset+=len(r)
                 paramQuery ={
                 'limit':2000,
                 'offset':offset
            ...
            while(paginated):
                 r2 = requests.get(url='https://metastats-api-v1.'+region+'.agiliumtrade.ai/users/current/accounts/'+account+'/historical-trades/'+startDate+startHora+'/'+endDate+endHora+'?updateHistory=true',headers=headers,params=paramQuery).json()
                 if 'trades' in r2:
            ...
            if ordenesTrades !=[]:
                for i in ordenesTrades:
                    ...
                    try:
                        ...
                        orden['open_price']=str(i['openPrice']) if "openPrice" in i else ""
                        orden['close_time']=i['closeTime']
                        orden['closed_price']=str(i['closePrice']) if "closePrice" in i else ""
                        orden['taxes']=str(i['taxes'])  if 'taxes' in i else '0.0'
                        orden['profit']=str(i['marketValue']) if "marketValue" in i else ""
                    except Exception as err:
                        #print(err)
                        pass
                    if ordenesDeals !=[]:
