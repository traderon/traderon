    ...
    if 'trades' in imported_trades:
        ...
        for trade_data in imported_trades['trades']:
            ...
            new_trade = Trades(user_id=userid, account_id=trade_data["account_id"], broker=trade_data["broker"], trade_id=trade_data["trade_id"], status=trade_data["status"], open_date=trade_data["open_date"],
                               symbol=trade_data["symbol"], entry=trade_data["entry"], exit=trade_data["exit"], size=trade_data["size"], pips=trade_data["pips"], ret_pips=trade_data["ret_pips"], ret=trade_data["ret"], ret_net=trade_data["ret_net"], side=trade_data["side"], setups=trade_data["setups"], mistakes=trade_data["mistakes"])
...
def get_trade_data():
    ...
    for data in data_display:
        ...
        data_array.append({"id": data.trade_id, "accountId": data.account_id, "broker": data.broker, "status": data.status, "openDate": data.open_date, "symbol": data.symbol,
                          "entry": data.entry, "exit": data.exit, "size": data.size, "pips": data.pips, "returnPips": data.ret_pips, "return": data.ret, "returnNet": data.ret_net, "side": data.side, "setups": data.setups, "mistakes": data.mistakes, "subs": subs})
